...
def get_activations(files, model, batch_size=50, dims=2048, device="cpu", num_workers=1, transforms=None):
    print(f"Number of files received: {len(files)}")
    if len(files) == 0:
        raise ValueError("No files were passed to get_activations, cannot proceed.")
    """Calculates the activations of the pool_3 layer for all images."""
    model.eval()

    if len(files) == 0:
        print("No files to process.")
        return np.array([])  # Return an empty array if no files found
    batch_size = min(50, len(files)) if len(files) > 0 else 1
...
#Allow for dataloader to access subfolders    
def compute_statistics_of_path(path, model, batch_size, dims, device, num_workers=1):
    if path.endswith(".npz"):
        with np.load(path) as f:
            m, s = f["mu"][:], f["sigma"][:]
        ...
        path = Path(path)
        #files = [file for ext in IMAGE_EXTENSIONS for file in path.rglob("*." + ext)]
        files = [file for ext in IMAGE_EXTENSIONS for file in path.rglob(f"*.{ext}")]
        print(f"Files being processed: {len(files)}")  # Print how many files are found
        if len(files) == 0:
